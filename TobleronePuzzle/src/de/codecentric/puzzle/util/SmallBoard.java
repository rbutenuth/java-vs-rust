...
public class SmallBoard implements Board {
    ...
    public SmallBoard(Game game, int pegs) {
        this.game = game;
        this.pegs = pegs;
    }

    private void setPeg(int row, int column) {
        pegs |= 1 << position(row, column);
    }
        ...
        if (isValid(r + 2 * y, c + 2 * x)) {
            ...
            if (isOccupied(r + y, c + x)) {
                ...
                if (!isOccupied(r + 2 * y, c + 2 * x)) {
                    SmallBoard next = new SmallBoard(game, pegs);
                    next.removePeg(r, c);
    ...
    public boolean equals(Object obj) {
        ...
        if (getClass() != obj.getClass())
            return false;
        SmallBoard other = (SmallBoard) obj;
        if (pegs != other.pegs)
            return false;
        return true;
    }
